...
class shinteticData:
    ...
    def generateData(self, samples=1000, inputFeatures=55, outputFeatures=3, commonLatent=2, 
        ...
        D0 = inputFeatures  # input features
        D1 = outputFeatures   # output features

        myKc = myKc

        K = commonLatent       # common latent variables
        K0 = firstViewLatent      # first view's latent variables
        K1 = secondViewLatent      # second view's latent variables
        ...
        self.__W_tot__ = np.vstack((W0,W1))

        # Generation of matrix Z
        self.__Z__ = np.random.normal(0.0, 1, Kc * N).reshape(N, Kc)

        # Generation of matrix X
        self.__X0__ = np.dot(self.__Z__,W0.T) + np.random.normal(0.0, 0.1, D0 * N).reshape(N, D0)
    ...
    def saveData(self, path):                    
        ...
        if fo.checkIfDirecotryExists(path):
            ...
            np.save (os.path.join(path, 'matrix0.npy'), self.__X0__)
            np.save (os.path.join(path, 'matrix1.npy'), self.__X1__)
        else:
...
class dataModel:
    ...
    def loadData(self, path):
        try:
            self.__W_tot__ = np.load (os.path.join(path, 'matriw.npy'))
            self.__Z__ = np.load (os.path.join(path, 'matriz.npy'))
            self.__X0__ = np.load (os.path.join(path, 'matrix0.npy'))
